...
import { loginStart, loginFailure, loginSuccess } from '../../redux/features/userSlice';
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (isValid) {
      ...
      try {
        ...
        dispatch(loginSuccess({ user: user as User, token }));
        toast.push({
          type: "success",
          title: t('login_form.login_success_title'),
          message: t('login_form.login_success_message', { username: user.username }),
        });
      } catch (error: any) {
        dispatch(loginFailure(error.message || t('login_form.login_failed')));
        toast.push({
          type: "error",
          title: t('login_form.login_failure_title'),
          message: error?.message || t('login_form.login_error_message'),
        });
      }
    }
  };
